...
import { destinationMomentScore, momentAllows } from "./moments.js";
...
export function normalizeDestination(destination) {
  return applyFamilyHassleCuration({
    ...
    bestTransportModeFromSp: destination.bestTransportModeFromSp || destination.best_transport_mode_from_sp || "",
    estimatedTotalMinutesFromSp: cleanNumber(destination.estimatedTotalMinutesFromSp ?? destination.estimated_total_minutes_from_sp),
    estimatedDistanceKmFromSp: cleanNumber(destination.estimatedDistanceKmFromSp ?? destination.estimated_distance_km_from_sp),
